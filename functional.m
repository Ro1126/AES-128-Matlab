% Deschidere fișiere și citire ca text
idk = fopen('Cheie.txt', 'r');
idt = fopen('text.txt', 'r');

% Citim conținutul ca șir de caractere
key_hex = fscanf(idk, '%s'); % Citește tot șirul hexazecimal
plaintext_hex = fscanf(idt, '%s');

fclose(idk);
fclose(idt);

% Convertim șirurile hexazecimale în vectori de octeți (uint8)
key = hex2dec(reshape(key_hex, 2, [])')'; % Reshape la perechi de 2 caractere și conversie
plaintext = hex2dec(reshape(plaintext_hex, 2, [])')';
plaintext=uint8(plaintext);
key=uint8(key);

disp('Input plaintext (hex):'+newline);
disp(plaintext_hex);
disp('Key (hex):');
disp(key_hex);
...
disp('Initial key state (hex):'+newline);
...
for i = 1:nr_blocks
    ...
    disp('Start of encryption (hex):'+newline);
    fprintf('%02x', state(:)');

    % Runda inițială AddRoundKey
    state = AddRoundKey(state, key_state);
    disp(' After initial AddRoundKey (hex):'+newline);
    fprintf('%02x', state(:)');

    % 10 runde (9 principale + 1 finală)
    for runda = 1:10
        disp(['Round ', num2str(runda), ':']+newline);
        % SubBytes
        disp('Before SubBytes (hex): '+newline);
fprintf('%02x ', state(:)');
        state = Subbyte(state);
        disp('  After SubBytes (hex): '+newline);
        fprintf('%02x', state(:)');

        % ShiftRows
        state = ShiftRows(state);
        disp(' After ShiftRows (hex): '+newline);
        fprintf('%02x', state(:)');

        % MixColumns (nu se aplică în runda 10)
        if runda ~= 10
            state = MixColumns(state);
            disp(' After MixColumns (hex): '+newline);
            fprintf('%02x', state(:)');
        end
            % Extinderea cheii
            key_state = roundkey(key_state, runda);
            disp('Round key (hex):'+newline);
            fprintf('%02x', key_state(:)');

            % AddRoundKey
            state = AddRoundKey(state, key_state);
            disp('After AddRoundKey (hex):'+newline);
            fprintf('%02x', state(:)');
        end
